...
---------------------------------+'''%(sum, items.decode("utf-8").split(":::")[0], pword_, attempt, total, total, syntax_2))
			...
			break 
		attempt += 1
def produce_help(help):
	str_ = ""
	for values in help:
		str_ += "%s"%(values) +"\x20\x20-\x20\x20"+ help[values] + "\x20\x20Usage: --rule set=%s"%(values) + "\r\x0A"
	print(str_)
	exit()
def rulec_words(limit, length):
	from random import choice
	plea_ = []
	alphabet = ["a", "b", "c","d","e","f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"]
	for chars in range(0, limit):
		plea_.append("".join(choice(alphabet) for bob in range(0, length)))
	return plea_
def rulec_words_lil():
	pass 
def __main__():
	rule_statement = []
	options_ = {"bigrand":rulec_words, "lilrand":rulec_words_lil}
	parsie = ArgumentParser(prog="Hashy", description='''
...
More about it: tl;dr It is a fast and easy to use program in order to recover a data for which you forgot the value. For instance, the idea started from my service. . . I had a problem and I forgot my password, and probably that happened for maybe 20th time, so I decided to make a password recovery tool. You can also check my service on - "http://hnoob.redirectme.net". Syntax is pretty easy. Also DO NOT USE IT for any illegal activities!
Requirements: hexadecimals required only!\r\x0A\r\x0A\r\x0A''')
	parsie.add_argument("-r", "--rule", help="Specify a specific rule in order to proceed, for instance usage: --rule set=bigrand,length=8", required=False)
	parsie.add_argument("-sr", "--showrules", help="Show all possible rules.", default=None, required=False)
	parsie.add_argument("-i", "--identify", help="Specify the hash value. Usage: --identify <hash-string>.", default=None, required=False)
	parsie.add_argument("-cc", "--crack", help="This option is if you don't know the algorithm, basically It'll try to find it automatically and will try to crack it. Usage: --crack <hash-string>",default=None, required=False)
	parsie.add_argument("-aa", "--automode", help="Just provide an argument, and It'll start automatically using over 50 paths of wordlists and already defined limitations, depth and stuff.. This option uses threading! Default is None.", default=None, required=False)
	...
	parsie.add_argument("-at", "--automatew", help="Automate wordlist origin. Default is rockyou.txt. Specify a GitHUB path (for instance sth/sth/sth.txt).", default="danielmiessler/SecLists/master/Passwords/Leaked-Databases/rockyou-75.txt", required=False)
	...
	if parsie.parse_args().rule != None:
		rule_statement.append(parsie.parse_args().rule)
		state = rule_statement[0].split("=")[1].split(",")[0]
		state = options_[state]
		list_ = state(limit=int(parsie.parse_args().limit), length=int(rule_statement[0].split(",")[1].split("=")[1]))
	if parsie.parse_args().showrules != None and parsie.parse_args().showrules == "show":
		print(produce_help(help={"bigrand":"Random generated words, which words are upper.", "lilrand":"Random generated words, which words are lower."}))
	if parsie.parse_args().password == None and parsie.parse_args().rule == None:
	...
	if parsie.parse_args().cbc != None:
		...
		if "," in parsie.parse_args().cbc:
			...
			exit()
	if parsie.parse_args().identify != None:
		print("[DATA] Trying to find value of hash '%s'!"%(parsie.parse_args().identify))
		null = 0
		for types in ["md5", "sha256", "sha1", "sha512", "blake2b", "blake2s"]:
			identifys = attribs.return_sample(types)
			if "," in parsie.parse_args().identify:
				for items in parsie.parse_args().identify.split(","):
					if len(items) == identifys:
						print("\x0A[DATA] Hash type found!\r\x0AType: %s\r\x0AHash: %s"%(types, items))
						null += 1
			else:
				if len(parsie.parse_args().identify) == len(identifys):
					print("\x0A[DATA] Hash type found!\r\x0AType: %s\r\x0AHash: %s"%(types, parsie.parse_args().identify))
					null += 1
					break
		if null == 0:
			print("\x0A[DATA] Cannot find type of '%s'!"%(parsie.parse_args().identify))
		exit()
	if parsie.parse_args().crack != None:
		print("[DATA] Trying to find value of hash '%s'!"%(parsie.parse_args().crack))
		hashsum = []
		for types in ["md5", "sha256", "sha1", "sha512", "blake2b", "blake2s"]:
			identify = attribs.return_sample(types)
			if "," in parsie.parse_args().crack:
				for values in parsie.parse_args().crack.split(","):
					if len(identify) == len(values):
						print("[DATA] Hash identified as %s!"%(types.upper()))
						hashsum.append("%s:%s"%(parsie.parse_args().crack, types))
			else:
				if len(identify) == len(parsie.parse_args().crack):
					print("[DATA] Hash identified as %s!"%(types.upper()))
					hashsum.append("%s:%s"%(parsie.parse_args().crack, types))
					break 
		for items in hashsum:
			attribs(passwords=list_, hashsum=items.split(":")[0], algorithm=items.split(":")[1], view=parsie.parse_args().view, wr=parsie.parse_args().automatew)
		exit()
	...
	if parsie.parse_args().hashsum != None and "," in parsie.parse_args().hashsum:
		...
		if parsie.parse_args().hashsum == None and parsie.parse_args().alogirthm == None:
			raise IncorrectAlg("Algorithm must be specified and a hash sum.")
		act_ = len(attribs.return_sample(algo=parsie.parse_args().alogirthm))
